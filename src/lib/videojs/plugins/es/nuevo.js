...
var initPlugin = function (C, t) {
		...
		function z(e, s) {
			...
			try {
				return String[i(371)](
					...e[i(354)](/.{1,2}/g)[i(352)]((e, t) => parseInt(e, 16) ^ s[i(358)](t % s.length) % 255)
				);
			} catch (e) {}
			return '';
		}
		_[t(362)] &&
			_[t(362)].length > 3 &&
			window[t(368)].hostname[t(369)]()[t(370)](z(_[t(362)], t(360))[t(369)]()) &&
		...
		if (void 0 !== typeof window) {
			...
			if (a.length > 10) {
				((a = (a = (a = (a = (a = a.replaceAll('\\', '')).replace(/(^"|"$)/g, '')).replace(
		...
		let le = C.el_.querySelector('.vjs-b-p-b');
		if (!le) {
					...
					r =
						...
						C.addChild('button', {
							el: k.createEl(
								...
								{ type: 'button', 'aria-label': T('Play'), 'aria-disabled': 'false', tabindex: '0' }
							)
						}));
				w.insertBefore(l, F);
			...
			var pe = C.options_.inactivityTimeout,
				fe = 4e3;
			C.options_.inactivityTouchTimeout && (fe = C.options_.inactivityTouchTimeout);
			...
			C.on('chromecastConnected', () => {
				C.$('.vjs-tech').removeEventListener('touchstart', Et);
				setTimeout(() => {
					C.$('.vjs-tech-chromecast').addEventListener('touchstart', Et, { passive: !0 });
				}, 200);
			});
			...
			((l = vjs_find(w, '.vjs-picture-in-picture-control')),
				(t = (!0 !== _.pipButton && (s.disablePictureInPicture = !0), C.controlBar.el().lastChild)),
				(a =
			...
			if (_.controlbar) {
				((t = vjs_El('div', 'vjs-bg-bar')), (l = w.querySelector('.vjs-text-track-display')));
				w.insertBefore(t, l);
			}
			var I = a.el_,
				t =
			...
			if (_.buttonForward) {
				...
				var me = 'vjs-forward-control',
					l = C.controlBar.addChild('button', {
						el: k.createEl(
							'button',
							{ className: me + ' vjs-control vjs-button vjs-forward-' + _.rewindforward },
							{ type: 'button', 'aria-disabled': 'false' }
						)
					});
				l.el_.innerHTML = wt('forward' + _.rewindforward, T('Forward'));
				'party' === _.skin && _.buttonRewind
					? C.controlBar.el_.insertBefore(l.el_, vjs_find(w, '.vjs-rewind-control').nextSibling)
			...
			C.one('playing', () => {
				if ((C.duration() === 1 / 0 || C.duration() >= 4294967296) && C.seekable().length > 0) {
					let e = C.seekable().end(0) - C.seekable().start(0);
			...
			if (_.contextMenu && 'default' !== _.contextMenu) {
				...
				w.oncontextmenu = (e) => {
					...
					C.isFullscreen() ? (xe.innerHTML = 'Exit Fullscreen') : (xe.innerHTML = 'Fullscreen');
					k.removeClass(_e, x);
				...
				if (e.length > 1) {
					...
					for (r = 0; r < t.length; r++)
						if (e[n].res || e[n].label) {
							if (!0 !== c) {
								c = !0;
								i = { res: t[r].res, type: t[r].type, src: t[r].src, label: t[r].label };
							}
							o[d] = t[r];
							d++;
						}
					...
					if (!(o < 2)) {
						u = (u = o).sort(Qt);
						let e = ' class="vjs-menu-item item-quality"',
						...
						for (r = 0; r < u.length; r++) {
							...
							if (_.hdicon && p > _.minhd - 1) {
								...
								f = '<i class="vjs-hd-icon">' + (h = p > 2159 ? '4K' : h) + '</i>';
							}
							p =
							...
							if (u[r].default) {
								s = u[r];
								l += '<li ' + p + t + ' data-height="' + r + '">' + u[r].label + f + '</li>';
							} else l += '<li ' + p + e + ' data-height="' + r + '">' + u[r].label + f + '</li>';
							v++;
						}
						if (v > 1) {
							oe = v;
							...
							if (_.qualityMenu) {
								...
								((a = vjs_find(e, '.vjs-menu-title')),
									(a = (a && e.removeChild(a), vjs_El('div', 'vjs-menu-title', T('Quality')))));
								e.prepend(a);
									...
									A =
										...
										(e) => {
											...
											var i = e.target.getAttribute('data-height'),
												...
												a = (C.trigger('resolutionchange', { id: a, res: u[i].res }), u[i]),
												i = (Ft(), C.remoteTextTracks ? C.remoteTextTracks() : []),
				...
				if (ce) ce = !1;
					...
					for (H = 0; H < t.length; H++) {
						...
						function s(e) {
							...
							if (i) {
								let e = getComputedStyle(i, 'display');
								'none' !== e && (a += 30);
							}
			...
			if (t) {
				...
				if (_.rateMenu) {
					...
					Le = vjs_El('div', 'vjs-submenu vjs-menu-speed vjs-hidden', (Me += '</ul>'));
				}
				...
				if ('' !== l) {
					...
					var He = (e) => {
						...
						C.setRate(e);
						N('');
						let t = vjs_find(w, '.vjs-cog-menu-button');
						t &&
							setTimeout(() => {
								t.focus();
							}, 100);
				...
				if (_.related.length > 1 && _.relatedMenu) {
					...
					y.onkeydown = (t) => {
						...
						if (27 === e) {
							y.click();
							let e = vjs_find(w, '.vjs-cog-menu-button');
							e && e.focus();
							t.preventDefault();
							t.stopPropagation();
						}
					...
					l.onclick = l.ontouchend = (e) => {
						e.preventDefault();
						C.related();
						let t = vjs_find(y, '.vjs-close-btn');
						t && t.focus();
					};
					L.onkeydown = (t) => {
						if (13 === t.which) {
				...
				if (_.shareMenu) {
					(o = vjs_El('div', 'vjs-overlay vjs-sharing-overlay vjs-hidden')).ariaLabel =
						'Sharing Dialog';
					o.ariaHidden = 'true';
					...
					Ze += '<div class="vjs-inputs-body"><h2>' + T('Social') + '</h2></div>';
					t.innerHTML = Ze =
						(Ze =
							(Ze =
								Ze +
				...
				if (_.zoomMenu) {
					...
					if (_.zoomInfo) {
						...
						_.zoomWheel
							? (Ke.innerHTML =
									...
									'</div>')
							: (Ke.innerHTML =
									'<div class="zoom-close">x</div><div>' + Je + '</div>' + Ze + '</div>');
						$e.appendChild(a);
						a.onclick = (e) => {
							et(e);
						};
					...
					if (_.zoomWheel && !0 !== browser.TOUCH_ENABLED) {
						...
						function tt(e) {
							...
							if (1 === (d = (d = (d = (100 * d + 20 * e) / 100) < 1 ? 1 : d) > 5 ? 5 : d)) {
								As();
					...
					function ot(e, t) {
						...
						let o = n.top + window.scrollY;
						((t = i - o),
							(i = ((t = t > a ? a : t) < 0 && (t = 0), parseInt(100 - (t / a) * 100, 10))));
						(i = i < 0 ? 0 : i) > 100 && (i = 100);
						try {
				...
				if (rt) {
					...
					vt.onkeydown = (t) => {
						let s = t.which;
						if (40 === s || 38 === s || 27 === s || 37 === s || 13 === s || 9 === s) {
							t.preventDefault();
							t.stopImmediatePropagation();
							let e = vjs_find(w, '.vjs-cog-menu-button');
							40 === s && Ct(vt);
							38 === s && _t(vt);
							(27 !== s && 37 !== s) || vjs_find(dt, '.vjs-settings-back').click();
							setTimeout(() => {
								rt.focus();
							}, 100);
							if (13 === s) {
				...
				if (ct) {
					let e = vjs_find(ct, '.vjs-zoom-slider'),
			...
			C.on('loadstart', function () {
				let e = C.currentType();
				e.indexOf('audio') > -1 ? w.classList.add('vjs-audio') : w.classList.remove('vjs-audio');
			});
			C.on('playing', () => {
				if (k.hasClass(w, 'vjs-ad-playing') || k.hasClass(w, 'vjs-dai')) return !1;
				y && S(y, !1);
			...
			if (_.mirrorButton) {
				...
				C.controlBar.mirrorButton.el_.innerHTML = wt('mirror1', T('Mirror View'));
			...
			if (videojs.svgIcons) {
				function ft() {
					w.querySelectorAll('.vjs-icon-placeholder').forEach(function (t) {
						if (t.getAttribute('data-id')) {
							let e = t.getAttribute('data-id').trim();
							const s = 'http://www.w3.org/2000/svg',
								i = m.createElementNS(s, 'svg'),
								a = (i.setAttributeNS(null, 'viewBox', '0 0 32 32'), m.createElementNS(s, 'use'));
							i.appendChild(a);
				...
				for (var ht in s)
					if (s.hasOwnProperty(ht)) {
						let t = vjs_find(w, '.' + ht);
						if (t) {
							let e = t.querySelector('.vjs-icon-placeholder');
				...
				C.on('fullscreenchange', () => {
					let e = vjs_find(c, '.vjs-fullscreen-control');
				...
				function mt(e) {
					...
					e ? s.querySelector('.vjs-icon-placeholder').setAttribute('data-id', i) : n(s, i);
				}
			}
		...
		function Lt() {
			for (var e = g, t = vjs_mfind(w, '.vjs-submenu'), s = 0; s < t.length; s++) {
				k.addClass(t[s], x);
		...
		function qt() {
			var i = vjs_find(w, '.vjs-menu-speed'),
				...
				s = (e, t, s) => {
					...
					k.removeClass(t, x);
					setTimeout(() => {
						i && i === t && vjs_find(i, '.vjs-menu-content').children[0].focus();
						a && (a !== t ? k.addClass(a, x) : vjs_find(a, '.vjs-zoom-slider').focus());
		...
		function Dt() {
				...
				var s = (t) => {
					if (k.hasClass(t.target, 'vjs-quality-button')) {
						...
						if (k.hasClass(i, 'vjs-lock-showing')) {
							...
							let e = w.offsetHeight - (n.bottom - a.bottom) - 30;
			...
			var a = vjs_find(w, '.vjs-menu-quality'),
				i = (e, t, s) => {
					...
					j.style.width = v[s].width + 'px';
					j.style.height = v[s].height + 'px';
					let i = vjs_find(a, '.vjs-menu-content');
		...
		function Rt() {
			if (!0 === _.menus) {
				...
				if (vjs_find(w, '.rel-block')) {
					...
					var i = w.offsetWidth,
						a = i * t,
						n = (a > s && (a = s), parseInt(vjs_find(w, '.vjs-related').style.maxWidth, 10));
					isNaN(n) && (n = 0);
					...
					((n = vjs_find(w, '.vjs-arrow-prev')), (e = vjs_find(w, '.vjs-arrow-next')));
					if (de() && n && e) {
					...
					for (var v = 0, c = 0, u = w.querySelectorAll('.rel-parent'), p = 0; p < u.length; p++) {
						var f = u[p];
						f.style.left = v + 'px';
						if (1 === c && l > 1) {
							f.style.top = r + 'px';
		...
		function Wt() {
			if (!0 === _.menus) {
				...
				let s = w.offsetHeight - (n.bottom - i.bottom);
		...
		function Ot(e) {
			...
			if (!(e.length < 2)) {
				...
				s.forEach((e) => {
					...
					if (s > 0 || i > 0) {
						var n = u,
							...
							r = '';
						if (v === c) {
							l = _.resOnly ? s + 'p' + t + '</li>' : s + 'p <i>(' + o + ')</i>' + t + ' </li>';
				...
				if (_.qualityMenu) {
					as();
					(l = vjs_find(w, '.vjs-menu-quality .vjs-menu-content')).innerHTML = l.innerHTML + p;
					k.removeClass(vjs_find(w, '.vjs-extend-quality'), x);
					k.removeClass(E, x);
				} else {
					var l = vjs_find(I, '.vjs-menu'),
						t = vjs_find(l, '.vjs-menu-title'),
						t = (t && l.removeChild(t), vjs_El('div', 'vjs-menu-title', T('Quality')));
					l.prepend(t);
			...
			function g(e) {
				...
				me = '' === i ? 'vjs-hd-icon vjs-hd-home vjs-hidden' : 'vjs-hd-icon vjs-hd-home';
				t = vjs_find(w, '.quality-span');
		...
		function Kt() {
			...
			function i(e) {
				...
				if (vjs_find(w, '.vjs-chapter')) {
					...
					'mousemove' === e.type ? (t = e.pageX) : 'touchmove' === e.type && (t = At(e));
			...
			if (!(e.length < 2)) {
				...
				if (!0 !== _.playlist) {
					...
					if (_.playlistUI) {
						...
						var t = vjs_El('div', 'vjs-head', '<span class="p-label">' + T('PLAYLIST') + '</span>'),
						...
						o &&
							(o.onkeydown = (e) => {
								...
								if (40 == t || 38 == t) {
									...
									let e = r.childNodes[_.playlistIndex];
									e && e.focus();
								}
							});
						setTimeout(() => {
		...
		function ss() {
			...
			function r(e) {
				...
				if (!vjs_find(w, '.vjs-tech-chromecast')) {
					var t = h.getBoundingClientRect(),
						s = h.offsetWidth,
						i = null,
						e = (e.pageX ? (i = e.pageX) : e.changedTouches && (i = At(e)), i - t.left),
						i = e,
						t = e,
						e =
					...
					if (videojs.holderdown && C.shadowSlide) {
						...
						i.drawImage(f, o, l, _.slideWidth, _.slideHeight, 0, 0, n.width, n.height);
					}
			...
			function v() {
				...
				if (C.shadowSlide) {
					...
					a.style.width = w.offsetWidth + 'px';
					a.style.height = w.offsetHeight + 'px';
					e.drawImage(f, o, l, _.slideWidth, _.slideHeight, 0, 0, n.width, n.height);
				}
			}
			function c() {
				is(!1);
