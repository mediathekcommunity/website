...
var vjs_pageTimer,
	...
	padRight = function (e, t, i = '0') {
		for (; e.length < t; ) e += i;
		return e;
	},
	padLeft = function (e, t, i = '0') {
	...
	fromBinary = function (e) {
		return parseInt(e, 2);
	},
	binaryIPv4 = function (e) {
		return e
			.split('.')
	...
	fromBinaryIPv4 = function (e) {
		return split(e, 8).map(fromBinary).join('.');
	},
	compressIPv6 = function (e) {
		var t = e.split(':'),
			i = 0,
			a = null,
			n = !1,
...
function sendbeacon(e, t) {
	...
	if ('' !== _edata.label && '' !== e)
		if (window.gtag)
			...
			window.ga('send', 'event', _edata.category, e, _edata.label, t, { transport: 'beacon' });
		else if (window._gaq)
			window._gaq.push([
				'_trackEvent',
				_edata.category,
		...
		else if (window._paq) window._paq.push(['trackEvent', _edata.category, e, _edata.label, t]);
}
var BufferTracking = function (e) {
		var t = this,
			...
			s = function () {
				r = !1;
				if (t.scrubbing() && (!e.bufferingConfig || !e.bufferingConfig.includeScrub)) {
					a = !0;
			...
			u = function () {
				if (!1 === r && !1 === a && t.currentTime() > 0) {
					r = new Date();
					n = +t.currentTime().toFixed(2);
				}
			},
			c = function () {
				var e = +t.currentTime().toFixed(2);
				if (r && e !== n) {
	...
	ResumeTracking = function (e) {
		...
		t.on('ended', r);
		t.on('play', function () {
			if (!(t.scrubbing() || t.seeking() || n || t.currentTime() < 1)) {
	...
	PercentileTracking = function (e) {
		...
		t.on('ended', function () {
			var e = this,
				t = e.duration();
			if (e.el_.className.indexOf('vjs-live') > -1) t = 0;
			if (_edata.uid) {
				e.trigger('track', {
					event: 'ended',
					sessionId: _edata.uid,
				...
				if (e.analytics) sendbeacon('completed', t);
				if (null !== _edata) sendscript(e, 'completed', t);
			}
		});
		t.on('fullscreenchange', function () {
			...
			if (e.isFullscreen()) {
				...
				if (e.analytics) sendbeacon('fullscreen', 1);
				if (null !== _edata) sendscript(e, 'fullscreen', 1);
			} else {
				if (!_edata.uid) return;
				e.trigger('track', {
				...
				if (e.analytics) sendbeacon('fullscreen', 0);
				if (null !== _edata) sendscript(e, 'fullscreen', 0);
			}
		});
	},
	SummaryTracking = function (e) {
		var t = this,
			...
			y = function () {
				...
				if (e.watchedDuration > 1) {
					...
					if (null !== _edata) sendscript(t, 'watchedTime', e.watchedDuration);
				}
				_();
			};
		if ('function' === typeof window.addEventListener) {
			var m = !1,
				b = function () {
					vjs_pageTimer = setTimeout(function () {
						y();
						_edata = null;
						vjs_currentSource = '';
					}, 1e4);
				...
				h = function (e) {
					if (!m) {
						...
						if ('visibilitychange' === e.type && 'hidden' === document.visibilityState) b();
					}
				};
		...
		t.on('timeupdate', function () {
			...
			if (0 !== e) {
				...
				l = t.currentTime().toFixed(0);
				_summaryData.watchedTime = o;
			}
		});
		t.on('loadeddata', function (e, i) {
			r = +t.duration().toFixed(0);
		});
		t.on('track', function (e, r) {
			switch (r.event) {
				case 'seek':
					i = r.seekCount;
					break;
	...
	PlayTracking = function (e) {
		var t = function () {
			...
			i = function () {
				clearTimeout(vjs_pageTimer);
				var e = this;
				if (vjs_currentSource !== e.currentSource()) {
					vjs_currentSource = e.currentSource();
					if (null !== _edata) {
						if (_edata.id)
							if ('function' === typeof e.video_id)
								if (_edata.id.length > 0)
									if (_edata.id === e.video_id()) {
										_summaryData.videoId = _edata.id;
										return;
									}
						if (_edata.label)
							if ('function' === typeof e.video_title)
								if (_edata.label.length > 0)
									if (_edata.label === e.video_title()) {
										_summaryData.videoTitle = _edata.label;
										return;
									}
					}
					_edata = {};
					if (void 0 !== e.trackingUrl) _edata.trackingUrl = e.trackingUrl;
					else _edata.trackingUrl = '';
					var t = Date.now();
					_edata.uid = _summaryData.sessionId = t;
					if ('function' === typeof e.video_title)
						if (e.video_title()) _edata.label = _summaryData.videoTitle = e.video_title();
					if ('function' === typeof e.video_id)
						if (e.video_id()) _edata.id = _summaryData.videoId = e.video_id();
					var i = e.el_.className;
					_edata.category = 'Video';
					if (e.isAudio()) _edata.category = 'Audio';
					...
					e.trigger('track', {
						...
						videoId: _edata.id,
						videoTitle: _edata.label,
						initialPlayTime: e.currentTime().toFixed(0)
					});
					sendscript(e, 'firstPlay', e.currentTime().toFixed(0));
					a(e);
					if (e.analytics) sendbeacon('firstPlay', e.currentTime().toFixed(0));
				}
			};
		function a(e) {
			function t(t, i) {
				...
				a.onreadystatechange = function () {
					if (4 === a.readyState && 200 === a.status)
						if (a.responseText) {
							...
							if (i.ip_address) if (e.anonymizeIP) i.ip_address = anonymizeIP(i.ip_address);
							i.browser = r();
							i.device = n();
							_edata.userInfo = i;
							if (!_edata.uid) return;
			...
			let i = '//ipapi.co/json/';
			if (e.abstractApiKey) i = '//ipgeolocation.abstractapi.com/v1/?api_key=' + e.abstractApiKey;
			else if (e.ipdataApiKey) i = '//api.ipdata.co?api-key=' + e.ipdataApiKey;
			t(i);
		}
		function n() {
			if ('undefined' !== window.orientation && void 0 !== window.orientation) return 'Mobile';
			else return 'Desktop';
		}
		function r() {
			if (-1 !== navigator.userAgent.indexOf('Chrome')) return 'Google Chrome';
			else if (-1 !== navigator.userAgent.indexOf('Firefox')) return 'Mozilla Firefox';
			else if (-1 !== navigator.userAgent.indexOf('MSIE')) return 'Internet Exploder';
	...
	SeekTracking = function (e) {
		var t = this;
		if (!(t.el_.className.indexOf('vjs-live') > -1)) {
			var i = 0,
				a = 0,
				n = function () {
					i = 0;
			...
			t.on('dispose', n);
			t.on('loadstart', n);
			var r = t.el().querySelector('.vjs-progress-holder');
			r.onmouseup = r.ontouchend = function () {
				a = t.currentTime().toFixed(2);
				i++;
	...
	ResolutionChange = function (e) {
		var t = this;
		t.on('resolutionchange', function (e, i) {
			if (_edata.uid)
				t.trigger('track', {
