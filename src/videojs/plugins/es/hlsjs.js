...
var registerSourceHandler = (videojs) => {
	var hooks = {};
	...
	function Html5Hlsjs(source, tech) {
		...
		function _initHlsjs() {
			...
			if (_hlsjsConfig.capLevelToPlayerSize === undefined)
				_hlsjsConfig.capLevelToPlayerSize = true;
			if (_hlsjsConfig.smoothQualityChange === undefined)
				_hlsjsConfig.smoothQualityChange = true;
			...
			if (_hlsjsConfig.capLevelToPlayerSize === true) {
				if (isFirefox || isChrome || isEdgeChromium) {
					...
					if (!_hlsjsConfig.maxMaxBufferLength)
						_hlsjsConfig.maxMaxBufferLength = 60;
				}
			}

			if (_hlsjsConfig.liveSyncDurationCount === undefined)
				_hlsjsConfig.liveSyncDurationCount = 3;
			...
			_hls.on(Hlsjs.Events.ERROR, (event, data) => {
			...
			_hls.on(Hlsjs.Events.LEVEL_LOADED, (event, data) => {
			...
			_hls.once(Hlsjs.Events.FRAG_LOADED, () => {
...
function nuevoHlsjsConfigHandler(options) {
	...
	if (!this.srOptions_.hlsjsConfig) {
		this.srOptions_.hlsjsConfig = options.hlsjsConfig;
	}

	if (!this.srOptions_.captionConfig) {
		this.srOptions_.captionConfig = options.captionConfig;
	}
}

var registerConfigPlugin = (videojs) => {
