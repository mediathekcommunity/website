<script lang="ts">
  ...
  let heroYouth: MediaItem[] = [];
  let otherYouth: MediaItem[] = [];
  ...
  onMount(async () => {
    try {
      ...
      // Assuming 'youth' is a genre or category in your media data
      const youthData: MediaItem[] = allMedia.filter((item: MediaItem) => item.genre === 'Youth');

      // Take the first few youth content for the hero section
      heroYouth = youthData.slice(0, 5); // Adjust the number as needed
      // Display all youth content in the grid below the hero section
      otherYouth = youthData;
    } catch (e: unknown) {
...
<main>
  {#if heroYouth.length > 0}
    <MediaCard heroItems={heroYouth} />
  {/if}

  {#if otherYouth.length > 0}
    <h1 class="text-3xl font-bold my-4 px-4">All Youth Content</h1>
    {#if loading}
    ...
    {:else}
      <div class="embla" use:EmblaCarouselSvelte={{ options: { loop: false, slidesToScroll: 'auto', containScroll: 'trimSnaps' }, plugins: [] }}>
        <div class="embla__container">
          {#each otherYouth as mediaItem (mediaItem.id)}
    ...
    {/if}
  {:else}
    <h1 class="text-3xl font-bold my-4 px-4">Youth Content</h1>
    {#if loading}
      <p class="px-4">Loading...</p>
    {:else if error}
      <p class="px-4">Error: {error}</p>
    {:else}
      <p class="px-4">No youth content found.</p>
    {/if}
  {/if}
</main>
